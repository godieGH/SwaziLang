...
// toml_tokenize_parse.sl
tumia reg kutoka "regex"

kazi tokenize toml_str:
  data tokens = [];

  data i = 0;
  wakati kweli:
    // if end of line stop the loop
    kama i > toml_str.urefu() {simama}
    ...
    // numbers (ints / floats / exponents / underscores)
...
kazi parse str:
  ...
  // add .expect helper on every token during parse-time
