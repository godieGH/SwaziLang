...
bool uv_module_has_active_handles() {
    ...
    return !g_uv_handles.empty();
}

void cleanup_uv_handles() {
    std::lock_guard<std::mutex> lock(g_uv_handles_mutex);

    // Close all handles (async operation)
    for (auto& [handle_ptr, shared] : g_uv_handles) {
        uv_handle_t* h = (uv_handle_t*)handle_ptr;
        if (!uv_is_closing(h)) {
            uv_close(h, nullptr);
        }
    }
    // DON'T clear here - let the loop finish closing first
}
