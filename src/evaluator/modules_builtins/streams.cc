...
static ObjectPtr create_stream_object(StreamEntryPtr entry) {
    ...
    auto on_impl = [entry](const std::vector<Value>& args, EnvPtr, const Token& token) -> Value {
        ...
        std::lock_guard<std::mutex> lk(entry->listeners_mutex);
        if (event == "data") entry->data_listeners.push_back(cb);
        else if (event == "end") entry->end_listeners.push_back(cb);
        else if (event == "error") entry->error_listeners.push_back(cb);
        else if (event == "close") entry->close_listeners.push_back(cb);
        else if (event == "drain") entry->drain_listeners.push_back(cb);
        else if (event == "finish") entry->finish_listeners.push_back(cb);
        else if (event == "pipe") entry->pipe_listeners.push_back(cb);
        else if (event == "unpipe") entry->unpipe_listeners.push_back(cb);
