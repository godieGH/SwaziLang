...
std::shared_ptr<ObjectValue> make_regex_exports(EnvPtr env) {
    ...
    auto compile_re = [](const std::string &pat, const std::string &flags) -> std::regex {
        auto opts = parse_regex_flags(flags);
        return std::regex(pat, opts);
    };

    // match(str, pattern [, flags]) -> bool (search anywhere)
    {
    ...
    // test(str, pattern [, flags]) -> bool (alias to match, kept for compatibility)
    {
    ...
    // search(str, pattern [, flags]) -> number (first match position) or -1
    {
        ...
        auto fn = make_native_fn("regex.replace", [compile_re](const std::vector<Value>& args, EnvPtr /*callEnv*/, const Token& token) -> Value {
            ...
            try {
                ...
                if (global) {
                    ...
                    std::smatch m;
                    if (std::regex_search(s, m, re)) {
                        std::string out = s.substr(0, m.position()) + std::regex_replace(m.str(), re, repl) + s.substr(m.position() + m.length());
                        return Value{ out };
                    }
                    return Value{ s };
                }
