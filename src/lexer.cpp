...
// Constructor
Lexer::Lexer(const std::string& source, const std::string& filename): src(source), filename(filename), i(0), line(1), col(1) {
...
// Scan backwards through emitted tokens but stop at a NEWLINE/INDENT/DEDENT:
// returns true if a control keyword (kama, kazi, kwa, wakati, fanya, etc.) appears
// on the same logical line before the '{' — then it's probably a block.
static bool same_line_has_control_keyword(const std::vector < Token>& out) {
   for (int idx = static_cast<int > (out.size()) - 1; idx >= 0; --idx) {
      TokenType t = out[idx].type;
      if (t == TokenType::NEWLINE || t == TokenType::INDENT || t == TokenType::DEDENT) {
         // reached start of logical line: no control keyword found on this line
         return false;
      }
      if (t == TokenType::KAMA // if || t == TokenType::KAZI // function def / kazi || t == TokenType::KILA || t == TokenType::KATIKA || t == TokenType::WHILE || t == TokenType::DOWHILE || t == TokenType::VINGINEVYO || t == TokenType::FOR // if you have FOR as loop keyword
      ) {
         return true;
      }
   }
...
void Lexer::scan_number(std::vector < Token>& out, int tok_line, int tok_col, size_t start_index) {
   ...
   while (!eof()) {
      ...
      if (std::isdigit((unsigned char)c)) {
         ...
         seen_dot = true; val.push_back(advance());
      }
      else break;
   }
...
void Lexer::scan_identifier_or_keyword(std::vector < Token>& out, int tok_line, int tok_col, size_t start_index) {
   ...
   TokenType > keywords = {
      ...
      // for-like loop with parentheses: kwa(a=0; a<6; a++) { ... } or kwa(a=0; a<6; a++): <INDENT> ...
      {
         "kila",
         TokenType::KILA
      },
      ...
      // while style: wakati a < 6: ...  or wakati a < 8 { ... }
      {
         "fanya",
         TokenType::DOWHILE
      },
      // do-while: fanya: <INDENT> ... wakati cond
      {
         "simama",
         TokenType::SIMAMA
      },
...
void Lexer::handle_newline(std::vector < Token>& out) {
   ...
   bool inside_object_literal = std::any_of(
      brace_stack.begin(), brace_stack.end(),
      [](BraceContext c) {
         return c == BraceContext::ObjectLiteral;
      }
   );

   if (paren_level == 0 && bracket_level == 0 && !inside_object_literal) {
...
void Lexer::scan_token(std::vector < Token>& out) {
   ...
   // comments: '#' or '//' style
   if (c == '#') {
      skip_line_comment(); return;
   }
   if (c == '/' && peek_next() == '/') {
      advance(); advance(); skip_line_comment(); return;
   }
   if (c == '/' && peek_next() == '*') {
      advance(); advance();
      while (!eof()) {
   ...
   // two-char operators
   if (c == '*' && peek_next() == '*') {
      add_token(out, TokenType::POWER, "**", line, col, 2); advance(); advance(); return;
   }
   if (c == '=' && peek_next() == '=') {
      add_token(out, TokenType::EQUALITY, "==", line, col, 2); advance(); advance(); return;
   }
   if (c == '=' && peek_next() == '>') {
      add_token(out, TokenType::LAMBDA, "=>", line, col, 2); advance(); advance(); return;
   }
   if (c == '!' && peek_next() == '=') {
      add_token(out, TokenType::NOTEQUAL, "!=", line, col, 2); advance(); advance(); return;
   }
   if (c == '>' && peek_next() == '=') {
      add_token(out, TokenType::GREATEROREQUALTHAN, ">=", line, col, 2); advance(); advance(); return;
   }
   if (c == '<' && peek_next() == '=') {
      add_token(out, TokenType::LESSOREQUALTHAN, "<=", line, col, 2); advance(); advance(); return;
   }
   if (c == '+' && peek_next() == '+') {
      add_token(out, TokenType::INCREMENT, "++", line, col, 2); advance(); advance(); return;
   }
   if (c == '-' && peek_next() == '-') {
      add_token(out, TokenType::DECREMENT, "--", line, col, 2); advance(); advance(); return;
   }
   if (c == '+' && peek_next() == '=') {
      add_token(out, TokenType::PLUS_ASSIGN, "+=", line, col, 2); advance(); advance(); return;
   }
   if (c == '-' && peek_next() == '=') {
      add_token(out, TokenType::MINUS_ASSIGN, "-=", line, col, 2); advance(); advance(); return;
   }
   if (c == '&' && peek_next() == '&') {
      add_token(out, TokenType::AND, "&&", line, col, 2); advance(); advance(); return;
   }
   if (c == '|' && peek_next() == '|') {
      add_token(out, TokenType::OR, "||", line, col, 2); advance(); advance(); return;
   }
   ...
   switch (c) {
      ...
      case '[':
      // entering bracketed expression (arrays, indexing, etc) -> suppress indent while inside
      bracket_level++;
      ...
      case '{': {
         // decide block vs object-literal by checking tokens on the same logical line
         bool as_block = same_line_has_control_keyword(out);
         brace_stack.push_back(as_block ? BraceContext::Block: BraceContext::ObjectLiteral);
   ...
   // number
   if (std::isdigit((unsigned char)c)) {
      size_t start_index = i; scan_number(out, line, col, start_index); return;
   }

   // identifier or keyword
   if (std::isalpha((unsigned char)c) || c == '_') {
      size_t start_index = i; scan_identifier_or_keyword(out, line, col, start_index); return;
   }
